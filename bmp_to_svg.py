...
def process_single_file(potrace_path, input_path, output_folder, cwd, potrace_args):
    """
    ...
    try:
        ...
        # 組合指令
        cmd = [
            potrace_path, str(source_to_trace), 
            "-s",                   # 輸出 SVG
            "-b", "svg",
            "-o", str(output_path)
        ] + potrace_args            # 插入動態參數
...
def main(args):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_single_file, potrace_path, f, to_folder, cwd, selected_args)
            for f in all_files
        ]

...
def append_config_encoding(file_path, unicode_field, alt_dict, ff_unicode_set):
    mycode = 0
    ...
    while x_line:
        ...
        if left_part == x_line[:left_part_length]:
            ...
            # do not overwrite the glyph exist.
            if is_mycode_in_alt_dict:
                alt_unicode_string = alt_dict[mycode]
                alt_unicode_int  = int(alt_unicode_string,16)
                if not alt_unicode_int in ff_unicode_set:
                    # target not exist.
                    #print("target not exist, do append")

                    output_file.write(new_line)
                    x_line = input_file.readline()
                    new_line = x_line

                    insert_new_alt = False
                    if left_part_alt != x_line[:left_part_alt_length]:
                        insert_new_alt = True
                    else:
                        # next line is AltUni2
                        if " %s." % (alt_unicode_string) in x_line:
                            # do nothing.
                            pass
                        else:
                            insert_new_alt = True
                    if insert_new_alt:
                        print("appending AltUni2 to file:%s" % (file_path))
                        insert_line = "AltUni2: %s.ffffffff.0\n" % (alt_unicode_string)
...
def scan_files(ff_folder, unicode_field, alt_dict, ff_unicode_set):
    my_set = set()
    my_dict = {}
    ...
    for f in files:
        ...
        if extension[1] == '.glyph':
            ...
            if exists(source_path):
                append_config_encoding(source_path, unicode_field, alt_dict, ff_unicode_set)
    ...
    if len(sys.argv)==argument_count:
        ...
        if len(source_ff) > 0:
            ...
            if exists(source_ff):
                ...
                scan_files(source_ff, unicode_field, alt_dict, ff_unicode_set)
    else:

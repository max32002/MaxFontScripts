...
def draw_character(char, font, canvas_size, x_offset=0, y_offset=0, auto_fit=True):
    ...
    if auto_fit:
        ...
        left = max(0, left - 5)
        upper = max(0, upper - 5)
        right = min(canvas_size * 2 - 1, right + 5)
        lower = min(canvas_size * 2 - 1, lower + 5)
        if left >= right or upper >= lower:
            return None
        img = np.array(img)[upper:lower, left:right]
        img = 255 - img
        img = Image.fromarray(img)
        img = transforms.ToTensor()(img).unsqueeze(0)
        pad_len = int(abs(img.shape[2] - img.shape[1]) / 2)
        fill_area = (0, 0, pad_len, pad_len) if img.shape[2] > img.shape[1] else (pad_len, pad_len, 0, 0)
        img = nn.ConstantPad2d(fill_area, 1)(img).squeeze(0)
        img = transforms.ToPILImage()(img).resize((canvas_size, canvas_size), Image.BILINEAR)
    else:
